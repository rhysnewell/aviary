#!/usr/bin/env python
###############################################################################
# binsnek.py - Info about binsnek.py
###############################################################################
#                                                                             #
...
def main():
    ...
    ############################ ~ Main Parser ~ ##############################
    main_parser = argparse.ArgumentParser(prog='binsnek',
    ...
    if (len(sys.argv) == 1 or sys.argv[1] == '-h' or sys.argv[1] == '--help'):
        ...
        if args.interleaved == "none":
            processor = BinSnek(args.assembly, args.pe1, args.pe2, args.longreads, int(args.max_threads), args.gtdb_path,
                                args.output, args.conda_prefix)
        elif args.pe2 == "none" and args.interleaved != "none":
            processor = BinSnek(args.assembly, args.interleaved, args.pe2, args.longreads, int(args.max_threads),
                                args.gtdb_path,
                                args.output, args.conda_prefix)
        elif args.longreads != "none":
            processor = BinSnek(args.assembly, args.pe1, args.pe2, args.longreads, int(args.max_threads),
                                args.gtdb_path,
                                args.output, args.conda_prefix)
        else:
...
class BinSnek:
    ...
    def run_workflow(self, workflow="recover_mags", cores=16, profile=None, dryrun=False, jobs=1, snakemake_args = ""):
        """Runs the BinSnek pipline
