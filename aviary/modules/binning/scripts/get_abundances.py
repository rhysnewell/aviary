...
if snakemake.config["long_reads"] != "none":
    if snakemake.config["long_read_type"][0] == "ont":
        subprocess.Popen("coverm genome -t %d -d bins/final_bins/ --single %s -p minimap2-ont --min-covered-fraction 0.0 -x fa %s --discard-unmapped > data/long_abundances.tsv" %
        ...
        subprocess.Popen("coverm genome -t %d -d bins/final_bins/ --single %s -p minimap2-pb --min-covered-fraction 0.0 -x fa %s --discard-unmapped > data/long_abundances.tsv" %
                         (snakemake.threads, " ".join(snakemake.config["long_reads"]),
                          '--bam-file-cache-directory data/binned_bams/' if snakemake.config['strain_analysis'] is True else ''), shell=True).wait()
    else:
        subprocess.Popen("coverm genome -t %d -d bins/final_bins/ --single %s -p minimap2-ont --min-covered-fraction 0.0 -x fa %s --discard_unmapped > data/long_abundances.tsv" %
                         (snakemake.threads, " ".join(snakemake.config["long_reads"]),
                          '--bam-file-cache-directory data/binned_bams/' if snakemake.config['strain_analysis'] is True else ''), shell=True).wait()

if snakemake.config['short_reads_2'] != 'none':
    subprocess.Popen("coverm genome -t %d -d bins/final_bins/ -1 %s -2 %s --min-covered-fraction 0.0 -x fa %s --discard-unmapped > data/short_abundances.tsv" %
    ...
    subprocess.Popen("coverm genome -t %d -d bins/final_bins/ --interleaved %s --min-covered-fraction 0.0 -x fa %s --discard-unmapped > data/short_abundances.tsv" %
